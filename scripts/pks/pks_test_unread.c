...
static int __init pks_test_init(void)
{
    ...
    // 设置AD(Access-Disable)位。AD位是第 2*PKEY 个bit。
    new_pkrs = original_pkrs | (1 << (TEST_PKEY * 2));
    write_pkrs_msr(new_pkrs);
    printk(KERN_INFO "Original PKRS: 0x%x, Set new PKRS to 0x%x to disable access for PKEY %d\n",
           original_pkrs, new_pkrs, TEST_PKEY);

    // 步骤 7: 尝试访问受保护的内存，并捕获预期的页错误
